...
async function renderStatus(user, range = "7d") { reviewMode = false; reviewId = null; const s = await api(`/api/study/status?range=${range}`); app.innerHTML = shell(`${topbar(user)}<section class="status-head"><div><span class="eyebrow">学习状态</span><h1>今天，继续向前。</h1></div><div class="lulu" aria-label="噜噜">噜</div></section><section class="status-controls"><button class="${s.range === "7d" ? "active" : ""}" id="d7">7D</button><button class="${s.range === "all" ? "active" : ""}" id="all">ALL</button></section><section class="metrics"><div><span>正确率</span><b>${s.accuracy == null ? "—" : `${Math.round(s.accuracy * 100)}%`}</b></div><div><span>作答数</span><b>${s.answeredCount}</b></div><div><span>平均用时</span><b>${s.averageAnswerDuration == null ? "—" : `${Math.round(s.averageAnswerDuration)} 秒`}</b></div></section>${trendView(s.trend)}<section class="status-actions"><div><b>${s.currentTraining.state === "ongoing" ? "有一组训练正在等你" : "从下一题开始"}</b><span>${s.currentTraining.state === "ongoing" ? "继续上次学习的位置" : "开始一组正式训练"}</span></div><button class="primary" id="continue-learning">${s.currentTraining.state === "ongoing" ? "继续学习" : "开始学习"}</button></section><section class="status-actions"><div><b>当前错题 ${s.wrongCountCurrent} 题</b><span>错题不受时间范围影响</span></div><button class="secondary" id="review">错题回顾</button></section><div class="secondary-actions"><button class="text-button" id="wrong-book">错题本</button><button class="text-button" id="manage">管理</button><button class="text-button" id="back">返回学习</button></div>`); app.querySelector("#d7").onclick = () => void renderStatus(user, "7d"); app.querySelector("#all").onclick = () => void renderStatus(user, "all"); app.querySelector("#continue-learning").onclick = async () => { if (s.currentTraining.state === "ready") await api("/api/study/start", "POST", {}); await load(); }; app.querySelector("#review").onclick = () => void startReview(user); app.querySelector("#wrong-book").onclick = () => void renderWrongBook(user); app.querySelector("#manage").onclick = () => void renderManage(user); app.querySelector("#back").onclick = () => void renderIdle(user); }
