...
function bindAccount(user) {
  ...
  if (account) account.onclick = async () => { if (reviewMode && reviewId) await api("/api/study/wrong-review/leave", "POST", { reviewId }); reviewMode = false; reviewId = null; await renderAccount(user); };
}
...
function renderReview(review, user = state.user) {
  ...
  bindAccount(user); app.querySelector("#leave-review").onclick = async () => { await api("/api/study/wrong-review/leave", "POST", { reviewId }); reviewMode = false; reviewId = null; renderIdle(user); }; app.querySelectorAll("[data-answer]").forEach(b => b.onclick = () => void submitReview(q.externalId, b.dataset.answer)); app.querySelector("#continue").onclick = () => void forward(); resumeTimer();
}
async function submitReview(questionExternalId, answer) { if (answered || busy) return; answered = true; pauseTimer(); const result = await api("/api/study/wrong-review/submit", "POST", { reviewId, questionExternalId, selectedAnswer: answer, submissionId: crypto.randomUUID(), answerDuration: Math.round(elapsed / 1000) }); app.querySelectorAll("[data-answer]").forEach(b => { b.disabled = true; if (b.dataset.answer === answer) b.classList.add(result.isCorrect ? "correct" : "wrong"); if (result.correctAnswer.includes(b.dataset.answer)) b.classList.add("correct"); }); app.insertAdjacentHTML("beforeend", `<section class="feedback"><strong>${result.isCorrect ? "回答正确，已移出错题本" : "回答错误，保留在错题本"}</strong><div>正确答案：${esc(result.correctAnswer.join("、"))}</div>${result.explanation ? `<div class="explanation"><b>解析</b><br>${esc(result.explanation)}</div>` : ""}</section>`); }
function trendView(trend) { const max = Math.max(1, ...trend.points.map(point => point.answeredCount)); return `<section class="trend"><div class="section-title"><b>学习趋势</b><span>${trend.granularity === "day" ? "每日" : trend.granularity === "week" ? "每周" : "每月"}</span></div><div class="trend-bars">${trend.points.map(point => `<div class="trend-point" title="${esc(point.label)}：${point.answeredCount} 题"><i style="height:${Math.max(5, point.answeredCount / max * 100)}%"></i><span>${esc(point.label.slice(trend.granularity === "month" ? 5 : 8))}</span></div>`).join("")}</div></section>`; }
async function renderStatus(user, range = "7d") { reviewMode = false; reviewId = null; const s = await api(`/api/study/status?range=${range}`); app.innerHTML = shell(`${topbar(user)}<main class="status-page"><header class="status-intro"><div><span class="eyebrow">学习状态</span><h1>我的学习近况</h1><p>不急，先看看你已经走了多远。</p></div><div class="status-controls"><button class="${s.range === "7d" ? "active" : ""}" id="d7">7D</button><button class="${s.range === "all" ? "active" : ""}" id="all">ALL</button></div></header><section class="lulu-stage"><div class="lulu-figure" aria-label="噜噜视觉位置"><i>噜</i><span></span><span></span></div><div class="learning-facts"><div><span>正确率</span><b>${s.accuracy == null ? "—" : `${Math.round(s.accuracy * 100)}%`}</b></div><div><span>作答</span><b>${s.answeredCount} <small>题</small></b></div><div><span>平均用时</span><b>${s.averageAnswerDuration == null ? "—" : `${Math.round(s.averageAnswerDuration)} 秒`}</b></div></div></section>${trendView(s.trend)}<section class="learning-actions"><button class="learning-action primary-action" id="continue-learning"><span>${s.currentTraining.state === "ongoing" ? "继续学习" : "开始学习"}</span><small>${s.currentTraining.state === "ongoing" ? "回到上次停下的位置" : "从下一题开始"}</small></button><button class="learning-action" id="review"><span>回顾错题 <em>${s.wrongCountCurrent}</em></span><small>错题不受时间范围影响</small></button></section><footer class="status-footer"><button class="text-button" id="wrong-book">查看错题本</button><button class="text-button" id="manage">管理</button><button class="text-button" id="back">返回学习</button></footer></main>`); app.querySelector("#d7").onclick = () => void renderStatus(user, "7d"); app.querySelector("#all").onclick = () => void renderStatus(user, "all"); app.querySelector("#continue-learning").onclick = async () => { if (s.currentTraining.state === "ready") await api("/api/study/start", "POST", {}); await load(); }; app.querySelector("#review").onclick = () => void startReview(user); app.querySelector("#wrong-book").onclick = () => void renderWrongBook(user); app.querySelector("#manage").onclick = () => void renderManage(user); app.querySelector("#back").onclick = () => void renderIdle(user); }
async function renderManage(user) { reviewMode = false; reviewId = null; app.innerHTML = shell(`${topbar(user)}<section class="card manage"><span class="eyebrow">管理</span><h1>学习资料与设置</h1><p class="muted">管理不打断学习；需要时再打开。</p><button class="secondary" id="manage-history">训练历史</button><button class="secondary" id="manage-special">专项训练</button><button class="secondary" id="manage-user">学习档案</button><button class="secondary" id="manage-bank">题库管理</button><button class="text-button" id="back">返回学习状态</button></section>`); app.querySelector("#manage-history").onclick = () => void renderHistory(user); app.querySelector("#manage-special").onclick = () => void renderSpecialModules(false); app.querySelector("#manage-user").onclick = () => void renderAccount(user); app.querySelector("#manage-bank").onclick = () => void renderBank(user); app.querySelector("#back").onclick = () => void renderStatus(user); }
async function renderBank(user) { const bank = await api("/api/study/bank-summary"); app.innerHTML = shell(`${topbar(user)}<section class="card manage"><span class="eyebrow">题库管理</span><h1>本地题库</h1><p class="muted">当前题库有 ${bank.questions} 道题，覆盖 ${bank.modules.length} 个模块。</p><div class="module-picker">${bank.modules.map(item => `<span>${esc(item.module)} · ${item.count}</span>`).join("")}</div><button class="text-button" id="back">返回管理</button></section>`); app.querySelector("#back").onclick = () => void renderManage(user); }
