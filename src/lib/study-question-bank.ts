...
const defaultQuestionBankFile = () => path.join(process.cwd(), "data", "question-bank.db");
export async function importQuestionBankV1(bank: Bank, file = defaultQuestionBankFile()) {
...
export async function getStudyQuestion(externalId: string, file?: string): Promise<StudyQuestion | undefined> {
  const db = await open(file ?? defaultQuestionBankFile()); const question = rows(db, "SELECT * FROM v1_questions WHERE externalId = ?", [externalId])[0]; if (!question) return undefined;
...
export async function getStudyQuestionsByModule(module: string, file?: string) { const db = await open(file ?? defaultQuestionBankFile()); return rows(db, "SELECT externalId FROM v1_questions WHERE module = ? ORDER BY externalId", [module]).map((row) => String(row.externalId)); }
export async function getRandomStudyQuestions(limit: number, file?: string) { const db = await open(file ?? defaultQuestionBankFile()); return rows(db, "SELECT externalId FROM v1_questions ORDER BY RANDOM() LIMIT ?", [limit]).map((row) => String(row.externalId)); }
export async function getAllStudyQuestionIds(file?: string) { const db = await open(file ?? defaultQuestionBankFile()); return rows(db, "SELECT externalId FROM v1_questions ORDER BY externalId").map((row) => String(row.externalId)); }
export async function getStudyBankSummary(file?: string) { const db = await open(file ?? defaultQuestionBankFile()); return { questions: Number(rows(db, "SELECT COUNT(*) AS count FROM v1_questions")[0].count), options: Number(rows(db, "SELECT COUNT(*) AS count FROM v1_options")[0].count), knowledgePoints: Number(rows(db, "SELECT COUNT(*) AS count FROM v1_knowledge_points")[0].count), materials: Number(rows(db, "SELECT COUNT(*) AS count FROM v1_materials")[0].count), mediaReferences: Number(rows(db, "SELECT COUNT(*) AS count FROM v1_question_media")[0].count), modules: rows(db, "SELECT module, COUNT(*) AS count FROM v1_questions GROUP BY module ORDER BY module").map((row) => ({ module: String(row.module), count: Number(row.count) })) }; }
