...
async function main() { const root = await fs.mkdtemp(path.join(os.tmpdir(), "gongkao-v1-import-")); const db = path.join(root, "study.db"); const bank = JSON.parse(await fs.readFile(path.resolve(process.cwd(), "fixtures/import-trials/fei98/question-bank-v1.json"), "utf8"));
