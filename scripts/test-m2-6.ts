...
async function main() {
  ...
  for (const hardening of ["overflow-x: hidden", "touch-action: manipulation", "@media (max-width: 380px)", ".profile { overflow: hidden"]) assert.ok(css.includes(hardening), `${hardening} protects narrow-screen layout and touch interaction`);
  ...
  for (const view of ["renderManage", "manage-flow", "renderHistory", "history-flow", "renderAccount", "profile-list", "renderBank", "bank-flow", "返回学习状态"]) assert.ok(js.includes(view), `${view} keeps the low-frequency management path renderable`);
  assert.ok(js.includes("还没有训练记录"), "history has a natural empty state"); assert.ok(js.includes("aria-current"), "current learning profile is explicit"); assert.ok(js.includes("is-invalidated"), "invalidated history remains distinguishable without changing it");
  assert.ok(js.includes("暂时没能完成操作") && !js.includes("error.message ||"), "request failures stay human-readable without raw technical errors"); assert.equal(js.includes("plannedQuestionCount"), false, "learning never exposes session progress");
