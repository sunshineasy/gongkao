...
async function main() {
  ...
  assert.match(css, /\.question-title/);
  ...
  assert.ok(source.includes("forwardArmed"), "forward requires a second downward intent");
  ...
  assert.ok(source.includes("result.explanation ?"), "explanation is conditional");
